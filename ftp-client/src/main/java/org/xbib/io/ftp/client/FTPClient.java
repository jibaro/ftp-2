...
public class FTPClient extends FTP implements Configurable {
     ...
     */
    public static final String FTP_SYSTEM_TYPE = "org.xbib.io.ftp.systemType";
     ...
     *
     */
    public static final String FTP_SYSTEM_TYPE_DEFAULT = "org.xbib.io.ftp.systemType.default";
    ...
    private FTPFileEntryParser fileEntryParser;
    ...
    public FTPClient() {
        ...
        entryParserFactory = new DefaultFTPFileEntryParserFactory();
    ...
    protected void _parsePassiveModeReply(String reply)
        ...
        passiveHost = m.group(1).replace(',', '.'); // Fix up to look like IP address
        ...
        if (__passiveNatWorkaroundStrategy != null) {
            try {
                String passiveHost = __passiveNatWorkaroundStrategy.resolve(this.passiveHost);
                if (!this.passiveHost.equals(passiveHost)) {
                    fireReplyReceived(0,
                            "[Replacing PASV mode reply address " + this.passiveHost + " with " + passiveHost + "]\n");
    ...
    protected Socket _openDataConnection_(String command, String arg)
        ...
        if (dataConnectionMode == ACTIVE_LOCAL_DATA_CONNECTION_MODE) {
            ...
            if (attemptEPSV && epsv() == FTPReply.ENTERING_EPSV_MODE) {
                _parseExtendedPassiveModeReply(replyLines.get(0));
            } else {
                if (isInet6Address) {
                ...
                _parsePassiveModeReply(replyLines.get(0));
            }
    ...
    protected void _connectAction_(Reader socketIsReader) throws IOException {
        ...
        if (__autodetectEncoding) {
            ArrayList<String> oldReplyLines = new ArrayList<String>(replyLines);
            int oldReplyCode = replyCode;
            if (hasFeature("UTF8") || hasFeature("UTF-8")) {
                setControlEncoding("UTF-8");
                bufferedReader =
                        new CRLFLineReader(new InputStreamReader(inputStream, getControlEncoding()));
                bufferedWriter =
                        new BufferedWriter(new OutputStreamWriter(outputStream, getControlEncoding()));
            }
            // restore the original reply (server greeting)
            replyLines.clear();
            replyLines.addAll(oldReplyLines);
            replyCode = oldReplyCode;
            newReplyString = true;
        }
    }
    ...
    public void setParserFactory(FTPFileEntryParserFactory parserFactory) {
        entryParserFactory = parserFactory;
    }
    ...
    public String printWorkingDirectory() throws IOException {
        ...
        return __parsePathname(replyLines.get(replyLines.size() - 1));
    }
     ...
     * @throws ConnectionClosedException                                    If the FTP server prematurely closes the connection
    ...
    void createParser(String parserKey) throws IOException {
        ...
        if (fileEntryParser == null || (parserKey != null && !entryParserKey.equals(parserKey))) {
            if (null != parserKey) {
                // if a parser key was supplied in the parameters,
                // use that to create the parser
                fileEntryParser =
                        entryParserFactory.createFileEntryParser(parserKey);
                ...
                if (null != ftpClientConfig && ftpClientConfig.getServerSystemKey().length() > 0) {
                    fileEntryParser = entryParserFactory.createFileEntryParser(ftpClientConfig);
                    ...
                    if (null != ftpClientConfig) { // system type must have been empty above
                        fileEntryParser = entryParserFactory.createFileEntryParser(new FTPClientConfig(systemType, ftpClientConfig));
                    } else {
                        fileEntryParser = entryParserFactory.createFileEntryParser(systemType);
                    }
    ...
    private FTPListParseEngine initiateListParsing(
        ...
        FTPListParseEngine engine = new FTPListParseEngine(parser, ftpClientConfig);
        if (socket == null) {
            return engine;
        }
    ...
    private FTPListParseEngine initiateMListParsing(String pathname) throws IOException {
        ...
        FTPListParseEngine engine = new FTPListParseEngine(MLSxEntryParser.getInstance(), ftpClientConfig);
        if (socket == null) {
            return engine;
        }
