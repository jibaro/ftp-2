...
public class NTFTPEntryParserTest extends CompositeFTPParseTestFramework {
    ...
    public void testParseFieldsOnDirectory() throws Exception {
        ...
        assertNotNull("Could not parse time", dir.getTimestamp());
        assertEquals("Thu Dec 05 17:03:00 1996", dir.getTimestamp().format(df) );
    ...
    public void testParseLeadingDigits() {
            ...
            assertNotNull("Could not parse time", file.getTimestamp());
            assertEquals("Thu May 22 00:08:00 1997", file.getTimestamp().format(df));

            FTPFile dir = getParser().parseFTPEntry("12-03-96  06:38PM       <DIR>           10 years and under");
            assertNotNull("Could not parse entry", dir);
            assertEquals("10 years and under", dir.getName());
            assertNotNull("Could not parse time", dir.getTimestamp());
            assertEquals("Tue Dec 03 18:38:00 1996", dir.getTimestamp().format(df));
    }
    ...
    public void testNET339() {
        ...
        assertEquals(5000000000L, file.getSize());
        assertNotNull("Could not parse time", file.getTimestamp());
        assertEquals("Thu May 22 12:08:00 1997", file.getTimestamp().format(df));

        FTPFile dir = getParser().parseFTPEntry("12-03-96  06:38       <DIR>           10 years and under");
        assertNotNull("Could not parse entry", dir);
        assertEquals("10 years and under", dir.getName());
        assertNotNull("Could not parse time", dir.getTimestamp());
        assertEquals("Tue Dec 03 06:38:00 1996", dir.getTimestamp().format(df));
    }
    ...
    public void testParseFieldsOnFile() throws Exception {
        ...
        assertEquals("Thu May 22 00:08:00 1997", f.getTimestamp().format(df));
        ...
        assertNotNull("Could not parse entry.", f);
        assertEquals("hour", 3,  f.getTimestamp().getHour());
    ...
    public void testDefaultPrecision() {
        testPrecision("05-26-1995  10:57AM               143712 $LDR$", TimeUnits.MINUTE);
        testPrecision("05-22-97  08:08                    828 AUTOEXEC.BAK", TimeUnits.MINUTE);
    }
